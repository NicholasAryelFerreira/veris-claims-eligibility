...
const BillAnalysis = z.object({
  ...
  dateOfService: z.string().describe("Date of service when present; otherwise the purchase date. Use an empty string when neither is visible."),
  servicesText: z.string(),
  totalCost: z.string(),
  discounts: z.string(),
  fieldConfidences: FieldConfidences.describe("Confidence in each canonical field result. Empty values can have high confidence when the model is confident the field is absent."),
    ...
    const content = [
      {
        ...
        text: [
          ...
          "For dateOfService, extract the date of service when present; otherwise extract the purchase date. Use ISO YYYY-MM-DD when possible.",
          ...
          "For fieldConfidences and extractedFields.confidence, return evidence-based confidence in the returned result from 0 to 100. If a requested field is absent and you are confident it is absent, return an empty value with high confidence. Use low confidence only when the document is unclear or unreadable.",
          "For ruleEvaluations, evaluate every non-empty eligibility rule line supplied by the reviewer exactly as written.",
          "The bill is eligible unless the visible document evidence proves otherwise. Return passed=false only when a rule violation is directly proven by visible bill text, line items, or a required field being absent.",
          "If a rule requires a field or fact to be present, and that required information is not visible after reading the document, return passed=false and identify the missing required information.",
      ...
      input: [
        {
          role: "system",
          content:
            "You extract medical bill eligibility review fields and apply only the reviewer-provided eligibility rules to visible bill evidence. The bill is eligible unless visible evidence directly proves a rule violation, including required information being absent. Do not invent independent medical coverage decisions or policy rules.",
        },
        {
          role: "user",
          content,
